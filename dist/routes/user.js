...
async function dashboardRoutes(fastify) {
    ...
    fastify.get('/api/dashboard/stats', async (request, reply) => {
        try {
            ...
            const applications = await pool.query('SELECT COUNT(*) FROM "Application" WHERE "userId" = $1', [userId]);
            const appliedToday = await pool.query(`SELECT COUNT(*) FROM "Application" 
         WHERE "userId" = $1 AND DATE("appliedAt") = CURRENT_DATE`, [userId]);
            const pending = await pool.query(`SELECT COUNT(*) FROM "Application" 
         WHERE "userId" = $1 AND status = 'pending'`, [userId]);
            const accepted = await pool.query(`SELECT COUNT(*) FROM "Application" 
         WHERE "userId" = $1 AND status = 'accepted'`, [userId]);
            const recent = await pool.query(`SELECT a.*, j.title as "jobTitle", j.company 
         FROM "Application" a 
         JOIN "Job" j ON a."jobId" = j.id 
         WHERE a."userId" = $1 
         ORDER BY a."appliedAt" DESC LIMIT 5`, [userId]);
            return reply.send({
...
async function userRoutes(fastify) {
    ...
    fastify.put('/api/user/profile', async (request, reply) => {
        try {
            ...
            const { fullName, bio, skills, experience, location } = request.body;
            // Update user name
            if (fullName) {
                await pool.query('UPDATE "User" SET "fullName" = $1 WHERE id = $2', [fullName, userId]);
            }
            // Upsert profile
            const result = await pool.query(`INSERT INTO "UserProfile" ("userId", bio, skills, experience, location)
         VALUES ($1, $2, $3, $4, $5)
         ...
         RETURNING *`, [userId, bio || '', skills || '', experience || '', location || '']);
    ...
    fastify.get('/api/user/settings', async (request, reply) => {
        try {
            ...
            const subscription = await pool.query('SELECT * FROM "Subscription" WHERE "userId" = $1', [userId]);
            return reply.send({
                settings: {
    ...
    fastify.put('/api/user/settings', async (request, reply) => {
        try {
            ...
            const { emailNotifications, weeklyDigest } = request.body;
            return reply.send({
