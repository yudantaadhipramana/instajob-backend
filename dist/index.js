...
const emailQueue_1 = require("./services/emailQueue");
const notificationQueue_1 = require("./services/notificationQueue");
const jobScrapingQueue_1 = require("./services/jobScrapingQueue");
...
const start = async () => {
    try {
        ...
        await (0, security_1.registerRateLimit)(fastify);
        // Auth Routes
        await fastify.register(auth_1.authRoutes);
        // Start Telegram Bot
        await (0, telegramBot_1.startBot)();
        ...
        const scheduleJobScraping = () => {
            ...
            if (hours % 6 === 0 && hours !== 0) {
                jobScrapingQueue_1.jobScrapingQueue.add('daily-scrape-linkedin', {
                    source: 'LinkedIn',
                    query: 'software engineer',
                });
                jobScrapingQueue_1.jobScrapingQueue.add('daily-scrape-indeed', {
                    source: 'Indeed',
                    query: 'software engineer',
                });
            }
        ...
        fastify.register(async (fastify) => {
            ...
            fastify.put('/api/user/profile', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
                    const { bio, skills, experience, location, resumeUrl } = req.body;
            ...
            fastify.get('/api/telegram/link-status', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
            ...
            fastify.post('/api/telegram/unlink', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
            ...
            fastify.post('/api/extension/sync-apply', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
                    const { title, company, location, description, url, isApplied } = req.body;
                    if (!title || !company)
                        return reply.code(400).send({ error: 'Title and company required' });
            ...
            fastify.get('/api/auto-apply/quota', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
                    const quota = await (0, rateLimit_1.getUserQuota)(userId);
                    return quota;
                }
                catch (err) {
            ...
            fastify.post('/api/auto-apply/queue', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
                    ...
                    // Check rate limit
                    const quotaStatus = await (0, rateLimit_1.canUserApply)(userId);
                    ...
                    await emailQueue_1.emailQueue.add('send-application', {
                        userId,
                        jobId,
                        jobTitle: job.title,
                        company: job.company,
                    ...
                    await notificationQueue_1.notificationQueue.add('notify', {
                        userId,
                    ...
                    // Increment quota
                    await (0, rateLimit_1.incrementApplyCount)(userId);
            ...
            fastify.get('/api/auto-apply/queue', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
            ...
            fastify.get('/api/referral/my-code', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
            ...
            fastify.post('/api/referral/redeem', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
                    ...
                    if (!referralCode)
                        return reply.code(400).send({ error: 'Referral code required' });
            ...
            fastify.get('/api/referral/leaderboard', async (req, reply) => {
                try {
            ...
            fastify.get('/api/referral/rewards', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
            ...
            fastify.get('/api/gamification/profile', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
            ...
            fastify.post('/api/gamification/check-in', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
            ...
            fastify.get('/api/gamification/achievements', async (req, reply) => {
                try {
                    const userId = req.user?.sub || req.user?.userId;
                    if (!userId)
                        return reply.code(401).send({ error: 'Unauthorized' });
        ...
        fastify.get('/api/jobs', async (req, reply) => {
            try {
                const { page = 1, search, remote, salaryMin, salaryMax } = req.query;
        ...
        fastify.get('/api/dashboard/stats', async (req, reply) => {
            try {
                const userId = req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
                const today = new Date();
                today.setHours(0, 0, 0, 0);
                const [totalJobs, appliedToday, saved, interviewed, offered] = await Promise.all([
                    prisma.job.count(),
        ...
        fastify.get('/api/analytics/overview', async (req, reply) => {
            try {
                const userId = req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
        ...
        fastify.get('/api/analytics/timeline', async (req, reply) => {
            try {
                const userId = req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
                ...
                applications.forEach(app => {
                    ...
                    if (app.status === 'interviewed')
                        timeline[date].interviewed++;
                    if (app.status === 'offered')
                        timeline[date].offered++;
        ...
        fastify.get('/api/analytics/industries', async (req, reply) => {
            try {
                const userId = req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
        ...
        fastify.get('/api/analytics/time-to-hire', async (req, reply) => {
            try {
                const userId = req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
        ...
        fastify.get('/api/subscription/status', async (req, reply) => {
            try {
                const userId = req.user?.sub || req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
        ...
        fastify.post('/api/subscription/upgrade', async (req, reply) => {
            try {
                const userId = req.user?.sub || req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
        ...
        fastify.post('/api/subscription/cancel', async (req, reply) => {
            try {
                const userId = req.user?.sub || req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
        ...
        fastify.get('/api/ai/match-score/:jobId', async (req, reply) => {
            try {
                const userId = req.user?.sub || req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
                const { jobId } = req.params;
                const score = await (0, aiService_1.calculateMatchScore)(userId, jobId);
                return { score };
            }
            catch (err) {
        ...
        fastify.get('/api/ai/recommendations', async (req, reply) => {
            try {
                const userId = req.user?.sub || req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
                const jobs = await (0, aiService_1.getJobRecommendations)(userId);
                // Populate match scores for recommendations
                const jobsWithScores = await Promise.all(jobs.map(async (job) => {
                    const score = await (0, aiService_1.calculateMatchScore)(userId, job.id);
        ...
        fastify.post('/api/ai/tailor-cover-letter', async (req, reply) => {
            try {
                const userId = req.user?.sub || req.user?.userId;
                if (!userId)
                    return reply.code(401).send({ error: 'Unauthorized' });
...
${user.fullName || 'InstaJob Candidate'}
        ...
        fastify.get('/api/admin/health', async (req, reply) => {
            try {
                const userId = req.user?.sub || req.user?.userId;
                if (!userId || !isAdmin(userId)) {
                    return reply.code(403).send({ error: 'Forbidden: Admin access required' });
