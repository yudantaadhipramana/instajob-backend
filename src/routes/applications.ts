...
export async function applicationRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/applications', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const result = await pool.query(
        `SELECT a.*, j.title as "jobTitle", j.company, j.location 
         FROM "Application" a 
  ...
  fastify.post('/api/applications', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const { jobId, notes } = request.body as any;
      
      // Check if already applied
      const existing = await pool.query(
        'SELECT id FROM "Application" WHERE "userId" = $1 AND "jobId" = $2',
        [userId, jobId]
      );
      ...
      const result = await pool.query(
        `INSERT INTO "Application" ("userId", "jobId", notes) 
  ...
  fastify.delete('/api/applications/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.code(401).send({ message: 'Unauthorized' });
